...
def pad_masks(masks: List[torch.tensor], max_objects: int):
    padded_masks = []
    for mask in masks:
        padded_mask = F.pad(mask[: max_objects], (0, 0, 0, 0, 0, max(0, max_objects - mask.shape[0])))
...
def pad_features(obj_features: torch.tensor, num_objects: List[int], max_objects: int):
    ...
    for num_obj in num_objects:
        ...
        final_tensors.append(F.pad(img_features[:max_objects], (0, 0, 0, max(0, max_objects - img_features.shape[0]))))
        cur += num_obj
